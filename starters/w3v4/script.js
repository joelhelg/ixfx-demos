...
const use = () => {
  ...
  if (withElement) {
    withElement.style.backgroundColor = isComplete ? hsl(100) : hsl(100);
    //withElement.style.marginBottom = isComplete ? "0px" : (scaled * multiplierY) + "px";
    withElement.style.marginLeft = isComplete ? "0px" : (scaled * 900) + "px";
    withElement.style.scale = isComplete ? "100%" : (100 + (scaled * 60)) + "%";

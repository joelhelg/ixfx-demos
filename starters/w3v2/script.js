...
const use = () => {
  ...
  const hsl = (v) => `hsl(360, ${v * 100}%, 100%)`; // Produces a hsl(60, sat%, lightness%) string
  ...
  if (withElement) {
    let negScaled = -0.5 + scaled
    withElement.style.backgroundColor = isComplete ? hsl(0) : hsl(scaled);
    //console.log(withElement.style.marginBottom)
    withElement.style.marginLeft = isComplete ? "0px" : (scaled * 750) + "px";
    //withElement.style.borderRadius = isComplete ? "0px" : scaled * 50 + "%";

    //withElement.style.scale = isComplete ? "73%" : (37 - (scaled * 70)) * 2 + "%";
    //withElement.style.rotate = isComplete ? "deg" : scaled * 1000 + "deg";
    ...
    if (negScaled > 0) {
      withElement.style.marginBottom = isComplete ? "730px" : (negScaled * 700) + "px";
      withElement.style.scale = isComplete ? "73%" : 50 + (negScaled * 100) + "%";
    } else {
      withElement.style.marginBottom = isComplete ? "730px" : (-negScaled * 700) + "px";
      withElement.style.scale = isComplete ? "73%" : 50 + (-negScaled * 100) + "%";
    }

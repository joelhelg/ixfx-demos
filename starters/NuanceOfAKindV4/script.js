...
const use = () => {
  ...
  const hsl = (v) => `hsl(${v * -15 + 50}, 85%, 50%)`; // Produces a hsl(60, sat%, lightness%) string
  ...
  if (withElement) {
    ...
    withElement.style.filter = isComplete ? "saturate(70%)" : `saturate(${(scaled * 30) + 70}%)`
    console.log(withElement.style.filter)
    console.log(scaled)
    //withElement.style.marginBottom = isComplete ? "0px" : (scaled * 650) + "px";
    withElement.style.scale = isComplete ? "30%" : (30 + (scaled * 5)) + "%";

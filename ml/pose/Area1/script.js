...
const computeHead = (pose) => {
  ...
  const leftWrist2 = getKeypoint(pose, `left_wrist`);
  const rightWrist2 = getKeypoint(pose, `right_wrist`);
  ...
  const xDistance = Points.distance(leftAnkle, leftWrist2);
  const yDistance = Points.distance(rightAnkle, rightWrist2);
  const area = (xDistance * yDistance);
...
const update = () => {
  ...
  if (heads.length >= 2) {
    ...
    background1.style.backgroundColor = `rgb(${255 - (armsY * (Math.random() * 255))} ${255 - (armsY * (Math.random() * 255))} ${255 - (armsY * (Math.random() * 255))})  `
